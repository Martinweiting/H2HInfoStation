      ...
      <div style={{ padding: 'clamp(28px, 4vw, 48px) clamp(24px, 5vw, 64px)' }}>
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: 12, lineHeight: 1.7, color: p.textSoft, maxWidth: 240 }}>
          ...
          {/* Links */}
          <div>
            <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, letterSpacing: '.24em', color: p.textSoft, marginBottom: 12 }}>
              PAGES
            </div>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[['\u6210\u54e1', '#members'], ['\u97f3\u6a02\u4f5c\u54c1', '#mv'], ['\u6642\u9593\u8ef8', '#discography'], ['\u884c\u7a0b', '#schedule'], ['\u7167\u7247', '#gallery'], ['Fan Wall', '#fanwall']].map(([l, h]) => (
                <li key={h}>
                  <a href={h} style={{ fontSize: 13, color: p.textSoft, transition: 'color .2s' }}
          ...
          <div>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[['Instagram', '#'], ['X (Twitter)', '#'], ['YouTube', '#'], ['TikTok', '#'], ['Weverse', '#']].map(([l, h]) => (
                <li key={l}>
                  <a href={h} aria-label={`${l} 官方帳號`} style={{ fontSize: 13, color: p.textSoft, transition: 'color .2s' }}
                    onMouseEnter={e => e.currentTarget.style.color = p.text}
                    onMouseLeave={e => e.currentTarget.style.color = p.textSoft}
                  >{l}</a>
                </li>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, letterSpacing: '.18em', color: p.textSoft }}>
              FAN-MADE · NOT OFFICIAL · FOR S2U
            </span>
            <Sparkles size={14} color="#FF8AA8" />
          </div>
          <span className="ff-script" style={{ fontSize: 18, color: '#FF8AA8' }}>
            made with love · S2U 💙
          </span>
        </div>
      </div>
